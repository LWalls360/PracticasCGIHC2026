...
void dibujaPata(float tX, float tZ, glm::mat4 model, GLint modelLoc);
...
int main() {
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::scale(model, glm::vec3(3.0, 0.2, 1.0));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		...
		dibujaPata(tX, tZ, model, modelLoc);
		dibujaPata(-tX, -tZ, model, modelLoc);
		dibujaPata(-tX, tZ, model, modelLoc);
		dibujaPata(tX, -tZ, model, modelLoc);
		dibujaPata(tX, -tZ/2, model, modelLoc);
 ...
 void dibujaPata(float tX, float tZ, glm::mat4 model, GLint modelLoc) {
	 model = glm::mat4(1.0f);
	 model = glm::translate(model, glm::vec3(tX, -0.1, tZ));
